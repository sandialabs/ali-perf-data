...
# Warmup
srun ${SRUNCONF} ${APPDIR}/Albany input_albany_MueLu.yaml >& out-ant16k20L.knl.warmup.txt
mv out-ant16k20L.knl.warmup.txt knl_timer_data/out-ant16k20L.knl.warmup.txt

# Run cases
for (( sid=0; sid<${NUMSAMPLES}; sid++ ))
do
  srun ${SRUNCONF} ${APPDIR}/Albany input_albany_MueLu.yaml >& out-ant16k20L.knl.${sid}.txt
  mv out-ant16k20L.knl.${sid}.txt knl_timer_data/out-ant16k20L.knl.${sid}.txt
done
